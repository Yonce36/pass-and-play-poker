...
### 計画変更(2026-07-19・ユーザー決定)

**Phase M3-C は実装しない（スコープ外）:**
- タイマー UI（SPEC 3.9 はデフォルトOFFのまま・store/UI連動は作らない）
- PIN 設定 UI（pinEntry/submitPin は既存のまま・設定画面から有効化しない）
- ピール開示ジェスチャー（長押しリビールを維持）

**現在の「次」:**
1. **ユーザー実機評価(M3-A/B 感触ラウンド1 + セキュリティ項目)**
2. フィードバック反映（触感・演出・バグ修正）
3. 任意: 素材（カード裏・アイコン・チップ）／安定化・UX微調整
4. （将来の別判断がなければ M3-C には進まない）
