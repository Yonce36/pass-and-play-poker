...
describe('selectHandCompleteView: fold勝ちを showdown と誤認して手札を公開しない（leak-auditor C-1）', () => {
  ...
  it('showdown(このハンドで busted になった敗者を含む)では参加者全員の手札と役が返る', () => {
    ...
    expect(w.isHandWinner).toBe(true);
    expect(l.amount).toBe(0);
    expect(l.cards).toEqual(['H3', 'D4']);
    expect(l.isHandWinner).toBe(false);
    expect(view.isChop).toBe(false);
  });

  it('同じ役でもキッカーが強い方だけ isHandWinner(誤チョップしない)', () => {
    // ボード AA + 低札。p1 は K キッカー、p2 は Q キッカー → 両方ワンペアだが p1 単独勝ち
    const board: Card[] = ['HA', 'CA', 'H2', 'C3', 'D4'];
    const p1: Player = {
      ...makePlayer('p1', 0, 1500, ['SK', 'C7']),
      status: 'active',
      totalContribution: 500,
    };
    const p2: Player = {
      ...makePlayer('p2', 1, 500, ['SQ', 'C8']),
      status: 'active',
      totalContribution: 500,
    };
    const s: GameState = {
    ...
    expect(view.isShowdown).toBe(true);
    expect(view.isChop).toBe(false);
    const e1 = view.entries.find((e) => e.playerId === 'p1')!;
    const e2 = view.entries.find((e) => e.playerId === 'p2')!;
    expect(e1.handRank).toBe('onePair');
    expect(e2.handRank).toBe('onePair');
    expect(e1.isHandWinner).toBe(true);
    expect(e2.isHandWinner).toBe(false);
    expect(e1.amount).toBe(1000);
    expect(e2.amount).toBe(0);
  });

  it('サイドポット返却で amount>0 が2人でも役勝ちが単独なら isChop=false', () => {
    // p1 が短い all-in(300)で役勝ち、p2 は超過拠出(700)のサイドを返却される
    // 旧UIは amount>0 が2人 → 誤って「チョップ」表示していた
    const board: Card[] = ['HA', 'CA', 'H2', 'C3', 'D4'];
    const p1: Player = {
      ...makePlayer('p1', 0, 300, ['SK', 'C7']), // ハンド後: メイン600を得て chips は別途。totalContribution でポット再構築
      status: 'allIn',
      totalContribution: 300,
      chips: 600,
    };
    const p2: Player = {
      ...makePlayer('p2', 1, 400, ['SQ', 'C8']),
      status: 'allIn',
    ...
    expect(e1.isHandWinner).toBe(true);
    expect(e2.isHandWinner).toBe(false);
    expect(e1.amount).toBe(600); // メインポット
    expect(e2.amount).toBe(400); // サイド返却のみ
    expect(e1.amount + e2.amount).toBe(1000);
  });
});

describe('リグレッション: runOutFrom（all-inランアウト演出用・UI専用フィールド）', () => {
  // 契約: applyAction が phase='showdown' を返し、かつアクション直前の communityCards が5枚未満のとき、
  // runOutFrom にはアクション前の公開枚数(0/3/4)が入る。それ以外(通常アクション・river通常showdown・
  // fold勝ち)は null。startNextHand 後も null に戻る。persist には含まれない。

  it('preflop 全員 all-in で showdown 直行したとき runOutFrom=0 になる(途中の all-in アクション時点では null)', () => {
    useGameStore.setState({ ...buildHeadsUpPreflop(), runOutFrom: 4 }); // 前ハンドの残骸を模したstale値

    useGameStore.getState().submitAction({ playerId: 'sb', type: 'allIn' });
    // まだ bb の手番が残っており showdown ではない → null にリセットされる
    expect(useGameStore.getState().runOutFrom).toBeNull();

    useGameStore.getState().submitAction({ playerId: 'bb', type: 'allIn' });
    // アクション前の公開コミュニティは 0 枚 → runOutFrom=0
    expect(useGameStore.getState().runOutFrom).toBe(0);
    expect(useGameStore.getState().communityCards).toHaveLength(5);
  });

  it('flop で全員 all-in になったとき runOutFrom=3(アクション前に公開されていた枚数)になる', () => {
    useGameStore.setState(buildHeadsUpPreflop());
    useGameStore.getState().submitAction({ playerId: 'sb', type: 'call' });
    useGameStore.getState().submitAction({ playerId: 'bb', type: 'check' }); // -> flop(3枚公開)
    expect(useGameStore.getState().communityCards).toHaveLength(3);

    useGameStore.getState().submitAction({ playerId: 'bb', type: 'allIn' });
    expect(useGameStore.getState().runOutFrom).toBeNull(); // sb の手番が残っている
    useGameStore.getState().submitAction({ playerId: 'sb', type: 'call' }); // 残額全部 → showdown直行
    expect(useGameStore.getState().runOutFrom).toBe(3);
    expect(useGameStore.getState().communityCards).toHaveLength(5);
  });

  it('showdown に到達しない通常アクションでは runOutFrom は null(stale値も消える)', () => {
    useGameStore.setState({ ...buildHeadsUpPreflop(), runOutFrom: 3 });
    useGameStore.getState().submitAction({ playerId: 'sb', type: 'call' }); // BBオプションが残りpreflop続行
    const s = useGameStore.getState();
    expect(s.phase).toBe('preflop');
    expect(s.runOutFrom).toBeNull();
  });

  it('river まで通常進行した showdown(アクション前に5枚公開済み)では runOutFrom は null', () => {
    useGameStore.setState(buildHeadsUpPreflop());
    useGameStore.getState().submitAction({ playerId: 'sb', type: 'call' });
    useGameStore.getState().submitAction({ playerId: 'bb', type: 'check' }); // -> flop
    useGameStore.getState().submitAction({ playerId: 'bb', type: 'check' });
    useGameStore.getState().submitAction({ playerId: 'sb', type: 'check' }); // -> turn
    useGameStore.getState().submitAction({ playerId: 'bb', type: 'check' });
    useGameStore.getState().submitAction({ playerId: 'sb', type: 'check' }); // -> river(5枚公開済み)
    useGameStore.getState().submitAction({ playerId: 'bb', type: 'bet', amount: 100 });
    useGameStore.getState().submitAction({ playerId: 'sb', type: 'call' }); // showdown -> handComplete

    const s = useGameStore.getState();
    expect(s.phase).toBe('handComplete');
    expect(s.runOutFrom).toBeNull(); // ランアウトなし(既に5枚公開されていた)
  });

  it('fold勝ちで handComplete に到達したときも runOutFrom は null', () => {
    useGameStore.setState({ ...buildHeadsUpPreflop(), runOutFrom: 0 });
    useGameStore.getState().submitAction({ playerId: 'sb', type: 'fold' });
    const s = useGameStore.getState();
    expect(s.phase).toBe('handComplete');
    expect(s.runOutFrom).toBeNull();
  ...
  it('startNextHand 後は null に戻る(前ハンドのランアウト値が残らない)', () => {
    // bb に多めのチップを持たせ、all-inランアウトで負けても busted せず handComplete で止まるようにする
    const preflop = postBlinds(
      makeState(
        [makePlayer('sb', 0, 1000, ['SA', 'DA']), makePlayer('bb', 1, 1500, ['H3', 'D4'])],
        'sb',
      ),
    );
    useGameStore.setState(preflop);

    useGameStore.getState().submitAction({ playerId: 'sb', type: 'allIn' });
    useGameStore.getState().submitAction({ playerId: 'bb', type: 'call' }); // sbがall-in → ランアウト

    const afterHand = useGameStore.getState();
    expect(afterHand.phase).toBe('handComplete'); // bb は 500 残るため gameOver にならない
    expect(afterHand.runOutFrom).toBe(0);

    useGameStore.getState().startNextHand(makeRandom(9));
    const s = useGameStore.getState();
    expect(s.phase).toBe('preflop');
    expect(s.runOutFrom).toBeNull();
  });

  it('persist の保存内容に runOutFrom が含まれない', () => {
    useGameStore.getState().startGame(['Alice', 'Bob'], undefined, makeRandom(1));
    useGameStore.setState({ runOutFrom: 3 });
    ...
    expect('runOutFrom' in persisted).toBe(false);
  });
});
