...
/** storage が使えない環境(SSR等)向けの何もしない StateStorage。アプリ側のフォールバック用 */
...
/**
 * ゲームストアを生成する。storage はプラットフォーム側が注入する
 * (Web: localStorage / React Native: AsyncStorage アダプタ)。
 * persist の partialize / merge(handoff除外・復元時locked強制)はここで一元管理し、
 * プラットフォーム側で上書きさせない(手札漏洩防止の要。CLAUDE.md 最優先事項)。
 */
export function createGameStore(getStorage: () => StateStorage) {
...
/** createGameStore が返すフック型(アプリ側の型注釈用) */
